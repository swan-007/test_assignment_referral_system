...
from .serializers import UserSerializer, PhonCodeSerializer, AlienInviteCodeSerializer
import random
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user_serializer.is_valid():
          ...
          # добовляем код из смс в БД
            ...
            return JsonResponse({'Status': True, 'Phone_code': simulator, 'id': user.id})
        else:
...
class ConfirmAccount(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if {'phone_code', 'phone_number'}.issubset(request.data):
            ...
            phone_code = PhonCode.objects.filter(user__phone_number=request.data['phone_number'],
                                            phon_code=request.data['phone_code']).first()
            #
            if phone_code:
                ...
                return JsonResponse({'Status': False, 'Errors': 'Неправильно указан номер или код подтверждения'})
        else:
...
class AccountDetails(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        user_info_dict = {'phone_number': serializer.data['phone_number'],
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer["alien_invite_code"]["code"].value:
            return JsonResponse({'Status': False, 'Error': 'There can be only one code'})

        # проверка данных
        if {'alien_invite_code'}.issubset(request.data):
            alien_invite_code_user = User.objects.filter(invite_code=request.data['alien_invite_code']).first()
            if alien_invite_code_user:
                ...
                if request.user.phone_number == alien_invite_code_user.phone_number:
                    return JsonResponse({'Status': False, 'Error': "You can't use your own code"})
                else:
                    # добовляем пользователя в request.data
                    request.data._mutable = True
                    request.data.update({"code_user": str(request.user.id),
                    ...
                    # проверка данных
                    alien_invite_code_serializer = AlienInviteCodeSerializer(data=request.data)
